...
def load_excel():
    ...
    # 학번 오름차순으로 정렬해서 저장
    student_file = pd.read_excel('성적관리프로그램/Entity/학생.xlsx').sort_values('학번')
    grade_assignment = pd.read_excel('성적관리프로그램/Entity/성적_과제.xlsx').sort_values('학번')
    grade_final = pd.read_excel('성적관리프로그램/Entity/성적_기말.xlsx').sort_values('학번')
    grade_middle = pd.read_excel('성적관리프로그램/Entity/성적_중간.xlsx').sort_values('학번')
    grade_attendance = pd.read_excel('성적관리프로그램/Entity/성적_출석.xlsx').sort_values('학번')
